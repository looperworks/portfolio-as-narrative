...
const MODULES = [
  {
    ...
    part: "Narrative",
    overview: `A portfolio is not a collection of coursework. It is a constructed argument. Every decision about what to include, how to sequence it, and where to place it on the page is an act of design. The portfolio is not separate from your work; it is your work, reframed for an audience. It also speaks before you do: before you enter a review, an interview, or a scholarship committee, the layout has already made your case or lost it. Reviewers typically spend thirty seconds on an initial scan. In that window, either you have earned a closer reading or you have not.

A portfolio is read at two speeds. At skim speed, the cover, image hierarchy, and project sequence must communicate a clear direction. At study speed (two to five minutes per project), the written statements, captions, and visual details must deepen that direction without contradicting it.

Different audiences read for different evidence. An academic reviewer looks for process: sketches, diagrams, failed iterations that reveal how you work through a problem. A large firm looks for resolution: polished deliverables and technical range that prove you can produce at a professional standard. A boutique studio looks for alignment: design sensibility and a point of view that signals you would be a collaborator, not just a pair of hands. Recognizing what each audience evaluates for is the difference between a page that documents and a page that argues.

Every architecture portfolio shares a common anatomy: cover, table of contents, project spreads, and supplemental material. Within each project spread, four image types perform distinct roles: concept, process, outcome, and context. Later modules develop these categories and their sequencing logic in detail. The governing principle is simple: once you can name the role each element plays, you stop arranging by habit and start composing by intent.`,
